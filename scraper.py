...
from api import get_lat_lon, get_weather, get_location
...
def get_detail_page():
    ...
    for link in links:
        try:
            ...
            row['location'] = get_location(metas[1])

            # Get geolocation data
            lat, lon = get_lat_lon(row['location'])
            if lat and lon:
                row['geolocation'] = {'latitude': lat, 'longitude': lon}

                # Get weather data
                weather = get_weather(lat, lon, row['date'])
                if weather:
                    for k in ['condition', 'temperature', 'wind']:
                        row[k] = weather[k]
                # exceed weather forecast date
                else:
                    break
            else:
...
def insert_to_pg():
    ...
    for url, row in zip(urls, data):
        q = '''
        INSERT INTO events (url, title, date, venue, category, location, lat, lon, condition, temperature, wind)
        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
        ON CONFLICT (url) DO NOTHING;
        '''
        cur.execute(q, (url, row['title'], row['date'], row['venue'], row['category'], row['location'], row['geolocation']['latitude'], row['geolocation']['longitude'], row['condition'], row['temperature'], row['wind']))
    print("Inset into db!")
